...
class UserRepository:
    ...
    def get_all(self) -> List[UserModel]:
        rows = self.session.fetch_all("SELECT id, firstName, lastName, email, dateOfBirth, roleId FROM users")
    ...
    def create(self, firstName: str, lastName: str, email: str, dateOfBirth: date, role_id: int):
        self.session.execute(
            "INSERT INTO users (firstName, lastName, email, dateOfBirth, roleId) VALUES (?, ?, ?, ?, ?)",
            (firstName, lastName, email, dateOfBirth.strftime("%Y-%m-%d"), role_id),
        )

    def get_by_id(self, user_id: int) -> UserModel:
        row = self.session.fetch_one(
            "SELECT id, firstName, lastName, email, dateOfBirth, roleId FROM users WHERE id = ?",
            (user_id,),
        )
        if row:
    ...
    def update(self, user_id: int, firstName: str, lastName: str, email: str, dateOfBirth: date, role_id: int):
        self.session.execute(
            "UPDATE users SET firstName = ?, lastName = ?, email = ?, dateOfBirth = ?, roleId = ? WHERE id = ?",
            (firstName, lastName, email, dateOfBirth.strftime("%Y-%m-%d"), role_id, user_id),
        )
